...
export async function GET(
    ...
    try {
        ...
        // Fetch rental data
        const [rental, rawProfits, swaps] = await Promise.all([
            client.readContract({
        ...
        // Parse profits safely
        const profits = rawProfits as any

        // Manual calculation to ensure accuracy
        const totalFeesEarned = BigInt(profits.totalFeesEarned)
        const rentalCostPaid = BigInt(profits.rentalCostPaid)

        // Ensure gas cost is positive (handle potential overflow or signed integer issues from contract)
        let gasCostEstimate = BigInt(profits.gasCostEstimate)
        if (gasCostEstimate < 0n) {
            gasCostEstimate = gasCostEstimate * -1n
        }

        // Calculate Net Profit: Fees - (Rental Cost + Gas)
        const totalCost = rentalCostPaid + gasCostEstimate
        const netProfit = totalFeesEarned - totalCost

        // Calculate ROI in basis points: (Net Profit / Total Cost) * 10000
        let roi = 0n
        if (totalCost > 0n) {
            roi = (netProfit * 10000n) / totalCost
        }

        // Construct corrected profit object
        const correctedProfits = {
            totalFeesEarned,
            rentalCostPaid,
            gasCostEstimate,
            netProfit,
        ...
        return NextResponse.json({
            ...
            profits: serializeData(correctedProfits),
