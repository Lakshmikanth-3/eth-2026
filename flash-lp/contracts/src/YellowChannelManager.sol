...
contract YellowChannelManager is Ownable, ReentrancyGuard {
    ...
    /// @notice Mapping from rental ID to channel ID (for Flash LP integration)
    mapping(uint256 => bytes32) public rentalChannels;
    ...
    /**
     * @notice Create a rental-specific channel
     * @param rentalId ID of the rental in FlashLP
     * @param renter Address of the renter
     * @param owner Address of the pool owner
     * @param duration Channel duration in seconds
     * @return channelId Unique identifier for the channel
     */
    function createRentalChannel(
        uint256 rentalId,
        address renter,
        address owner,
        uint256 duration
        ...
        require(renter != address(0) && owner != address(0), "Invalid addresses");
        require(rentalChannels[rentalId] == bytes32(0), "Rental channel exists");
        ...
        participantChannels[renter].push(channelId);
        participantChannels[owner].push(channelId);
        rentalChannels[rentalId] = channelId;
        totalChannels++;
        
        emit ChannelOpened(channelId, renter, owner, msg.value, block.timestamp + duration);
    }
    
    /**
     * @notice Get channel ID for a rental
     * @param rentalId ID of the rental
     * @return channelId Channel ID associated with rental
     */
    function getRentalChannel(uint256 rentalId) 
        external 
        view 
        returns (bytes32) 
    {
        return rentalChannels[rentalId];
    }
}
