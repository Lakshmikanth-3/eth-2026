...
contract UnifiedFlashLP is ERC721, Ownable, ReentrancyGuard {
   ...
   mapping(uint256 => bool) public supportedChains;
    ...
    struct Rental {
        ...
        uint256 swapCount;
        uint256 feesEarned;
        bytes32 channelId;        // ✅ Added for Yellow Network
    }
    ...
    constructor(address _yellowChannelManager) 
    ...
    /**
     * @notice Settle rental using Yellow Network channel state
     * @param rentalId ID of the rental to settle
     * @param finalBalance1 Final balance for renter (from Yellow channel)
     * @param finalBalance2 Final balance for owner (from Yellow channel)
     * @param nonce Final state nonce
     * @param signature1 Renter's signature on final state
     * @param signature2 Owner's signature on final state
     */
    function settleRentalWithYellow(
        uint256 rentalId,
        ...
        bytes32 channelId = rental.channelId;
        require(channelId != bytes32(0), "No Yellow channel");
        
        // Close the Yellow channel with final state
        yellowChannelManager.closeChannel(
            channelId,
            finalBalance1,
            finalBalance2,
            nonce,
        ...
        // Calculate fees based on Yellow channel balances
        // finalBalance2 represents fees earned by owner
        uint256 platformFee = (finalBalance2 * platformFeeBps) / 10000;
        ...
        // Owner receives their portion
        if (ownerAmount > 0) {
            payable(rental.poolOwner).transfer(ownerAmount);
        }
        
        // Renter keeps their remaining balance
        if (finalBalance1 > 0) {
            payable(rental.renter).transfer(finalBalance1);
        }
        
        emit RentalEnded(rentalId, finalBalance2);
    }
    
    /**
     * @notice Update Yellow Channel Manager address
     * @param _newManager New YellowChannelManager address
     */
    function setYellowChannelManager(address _newManager) external onlyOwner {
        require(_newManager != address(0), "Invalid address");
        yellowChannelManager = YellowChannelManager(_newManager);
    }
}
